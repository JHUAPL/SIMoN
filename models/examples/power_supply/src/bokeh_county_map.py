...
def CountyMap(county_data, name):
    ...
    county_rates = [0 if math.isnan(x) else x for x in county_rates]
    data=dict(
    x=county_xs,
    y=county_ys,
    name=county_names,
    ...
    tooltips=[
    ("Name", "@name"), ("State", "@state"), (name, "@rate{int}"),("(Long, Lat)", "($x, $y)")
    ])
    ...
    return file_html(p, CDN, title=name)
